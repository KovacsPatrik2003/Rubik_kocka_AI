...
class CubeDisplay(Entity):  # A CubeDisplay osztály öröklődik az Entity osztályból
    ...
    def create_cube_positions(self):
        self.LEFT = {(-1, y, z) for y in range(-1, 2) for z in range(-1, 2)}
        self.BOTTOM = {(x, -1, z) for x in range(-1, 2) for z in range(-1, 2)}
        self.FACE = {(x, y, -1) for x in range(-1, 2) for y in range(-1, 2)}
